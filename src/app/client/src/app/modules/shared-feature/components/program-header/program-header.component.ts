...
export class ProgramHeaderComponent implements OnInit, OnDestroy {
  ...
  constructor(public toasterService: ToasterService, public resourceService: ResourceService,
     ...
     public programsService: ProgramsService, public programStageService: ProgramStageService) { }

  ngOnInit() {
    this.generateHeaderData();
    this.setActiveDate();

    this.programsService.selectedCollections$.subscribe(
      data => {
  ...
  canNominate() {
    const today = moment();
    return moment(this.programDetails.nomination_enddate).isSameOrAfter(today, 'day');
  }
